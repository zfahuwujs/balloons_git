...
|	Core functions for the SagePay Gateway	
...
function fixedVars(){
	...
	include_once("includes/sessionStart.inc.php");			
			$delIsoCode = $db->select("SELECT iso FROM  ".$glob['dbprefix']."CubeCart_iso_countries WHERE id = '".mysql_real_escape_string($basket['delInf']['country'])."' OR printable_name = '".mysql_real_escape_string($basket['delInf']['country'])."'");
			
			if($delIsoCode==TRUE)
			{
				$del_iso = $delIsoCode[0]['iso'];
			}else{
				$del_iso = 0;
			}
			
			$billIsoCode = $db->select("SELECT iso FROM  ".$glob['dbprefix']."CubeCart_iso_countries WHERE id = '".mysql_real_escape_string($ccUserData[0]['country'])."' OR printable_name = '".mysql_real_escape_string($ccUserData[0]['country'])."'");
			if($billIsoCode==TRUE)
			{
				$bill_iso = $billIsoCode[0]['iso'];
			}else{
				$bill_iso = 0;
			}
			//check postcode
			 $bill_poscode = substr(preg_replace('/[^a-zA-Z0-9 -]/', '', $ccUserData[0]['postcode']), 0, 10);	
			 
			 $del_poscode = substr(preg_replace('/[^a-zA-Z0-9 -]/', '', $basket['delInf']['postcode']), 0, 10);	
			//===============
			
			if($del_iso=='US')
			{
				$delState = substr($basket['delInf']['county'], 0, 2);
			}
			if($bill_iso=='US')
			{
				$billState = substr($ccUserData[0]['county'], 0, 2);
			}
			...
			$cryptVars = 
				...
				."&BillingAddress2=".$ccUserData[0]['add_2']
				
				."&BillingCity=".$ccUserData[0]['town']
				
				."&BillingCountry=".$bill_iso
				
				."&BillingState="//.$ccUserData[0]['county']
				...
				."&DeliveryAddress2=".$basket['delInf']['add_2']
				
				."&DeliveryCity=".$basket['delInf']['town']
				
				."&DeliveryCountry=".$del_iso
				
				."&DeliveryState="//.$basket['delInf']['county']
				
				."&DeliveryPostCode=".$basket['delInf']['postcode']
				
				."&DeliverySurname=".$basket['delInf']['lastName']
				
				."&DeliveryFirstnames=".$basket['delInf']['firstName']

				."&ContactNumber=".$ccUserData[0]['phone']

//				."&ApplyAVSCV2=0&Apply3DSecure=0&ShoppingBasket=ON"  We have taken our the ShoppingBasket Variable - It seemed SagePay didn't need it.

				."&ApplyAVSCV2=0&Apply3DSecure=0"

				."&SuccessURL=".$GLOBALS['storeURL']."/modules/gateway/SagePay/confirmed.php?oid=".base64_encode($cart_order_id)."&amt=".base64_encode($basket['grandTotal'])

				."&FailureURL=".$GLOBALS['storeURL']."/modules/gateway/SagePay/confirmed.php?f=1";
...
if($module['gate'] == "sim") {
	...
	$formAction = "https://test.sagepay.com/Simulator/VSPDirectGateway.asp";
