...
if($_GET['process']==1){
	...
	$currencyInfo = explode("|",$module['currency']);
	?>
	<html>
	<head></head>
	...
	<input type="hidden" name="CcpaClientId" value="<?=$module['userAlias'];?>01" />
	<input type="hidden" name="CurrencyExponent" value="2" />
	<input type="hidden" name="PurchaseAmount" value="<?=$currencyInfo[0].$_POST["amount"];?>" />
	<input type="hidden" name="PurchaseAmountRaw" value="<?=(100*$_POST["amount"]);?>" />
	<input type="hidden" name="PurchaseCurrency" value="<?=$currencyInfo[1];?>" />
