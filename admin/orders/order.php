...
if(isset($_GET['reset']) && $_GET['reset']>0){
		...
		//Update Admin Order Changes
		$auditData['username'] = $db->mySQLsafe($ccAdminData[0]['username']);
		$auditData['time'] = time();
		$auditData['ipAddress'] = $db->mySQLsafe($_SERVER['REMOTE_ADDR']);
		$auditData['type'] = $db->mySQLsafe("Download Reset");
		$auditData['cart_order_id'] = $db->mySQLsafe($_GET['cart_order_id']);
	
		$audit = $db->insert($glob['dbprefix']."CubeCart_admin_sessions_new", $auditData);
		//End Update Admin Order Changes
...
if(isset($_POST['ship_date'])){
	...
	if($update){
		...
		if($_POST['ship_date'] == ''){
			$shipping = "No shipping date set";
		}else{
			$shipping = $db->mySQLsafe($_POST['ship_date']);
		}
		
		$auditData['username'] = $db->mySQLsafe($ccAdminData[0]['username']);
		$auditData['time'] = time();
		$auditData['ipAddress'] = $db->mySQLsafe($_SERVER['REMOTE_ADDR']);
		$auditData['type'] = $db->mySQLsafe("<strong>Order Status: </strong>".$status."<br /><strong>Shipping Date:</strong> ".$shipping);
		$auditData['cart_order_id'] = $db->mySQLsafe($_GET['cart_order_id']);
...
<p>
  <input type="button" class="submit" onclick="openPopUp('print.php?cart_order_id=<?php echo $_GET['cart_order_id']; ?>', 'PrintSlip', 600, 550, 1)" value="<?php echo $lang['admin']['orders']['print_packing_slip'];?>" />
</p>
<?php
...
<table border="0" width="100%" cellspacing="0" cellpadding="4" class="mainTable">
  ...
  <tr>
    <td align="left" valign="top" class="copyText"><strong><?php echo $lang['admin']['orders']['invoice_address']; ?></strong>
      <div style="padding: 10px;"><?php echo $result[0]['name']; ?><br/>
        ...
        <?php echo countryName($result[0]['country']); ?></div></td>
    <td align="left" valign="top" class="copyText"><strong><?php echo $lang['admin']['orders']['delivery_address']; ?></strong>
      <div style="padding: 10px;"><?php echo $result[0]['name_d']; ?><br/>
        ...
        <?php echo $result[0]['country_d']; ?></div></td>
  </tr>
  <tr>
    ...
    <td align="left" valign="top" class="copyText"><strong><?php echo $lang['admin']['orders']['customer_comments'];?></strong>
      <div style="padding: 10px;">
        <?php if(empty($result[0]['customer_comments'])) { echo $lang['admin']['orders']['no_comments_left']; } else { echo "&quot;".$result[0]['customer_comments']."&quot;"; } ?>
      </div></td>
  </tr>
  <tr>
  ...
  ?>
  <tr>
    <td colspan="2" align="left" valign="bottom" style="padding: 0px;"><?php
	...
	$update = $db->misc($query);
	 ...
	 ?>
        <tr>
          <td class="<?php echo $cellColor; ?>"><span class="copyText"><?php echo $results[$i]['name']; ?><br />
		...
		?>
            <br />
            <strong><?php echo $lang['admin']['orders']['download_link']; ?></strong> [<a href="order.php?cart_order_id=<?php echo $_GET['cart_order_id']; ?>&amp;reset=<?php echo $download[0]['id'];?>" class="txtLink" onclick="alert('<?php echo $lang['admin']['orders']['warn_reset'];?>')"><?php echo $lang['admin']['orders']['reset_link'];?></a>] <br />
            <?php echo $glob['storeURL']."/download.php?pid=".$download[0]['productId']."&oid=".base64_encode($_GET['cart_order_id'])."&ak=".$download[0]['accessKey'];?> <br />
        ...
        <tr>
          ...
          <td align="right" class="btmSubNav"><span class="copyText"><?php echo priceFormat($result[0]['subtotal']);?></span> </td>
        </tr>
        <tr>
          ...
          <td align="right"><span class="copyText"><font color="red">-<?php echo priceFormat($result[0]['discountTot']);?></font></span> </td>
        </tr>
        <tr>
          <td><span class="copyText"><?php echo $lang['admin']['orders']['total_tax'];?></span></td>
          <td align="right"><span class="copyText"><?php echo priceFormat($result[0]['total_tax']);?></span> </td>
        </tr>
        <tr>
          <td><span class="copyText"><?php echo $lang['admin']['orders']['shipping'];?></span></td>
          <td align="right"><span class="copyText"><?php echo priceFormat($result[0]['total_ship']);?></span> </td>
        </tr>
        <tr>
        ...
        <td style="border-top: 1px solid #000000;">&nbsp;</td>
          ...
          <td align="right" class="btmSubNav" style="border-top: 1px solid #000000;"><span class="copyText"><strong><?php echo priceFormat($result[0]['prod_total']);?></strong></span></td>
        </tr>
      </table></td>
  </tr>
  <?php
...
<form method="post" action="<?php echo $GLOBALS['rootRel'];?>admin/orders/order.php?cart_order_id=<?php echo $_GET['cart_order_id']; ?>" name="modify" enctype="multipart/form-data">
  <table border="0" align="center" cellpadding="3" cellspacing="0" class="mainTable">
    ...
    <tr>
      <td width="250" class="copyText"><strong><?php echo $lang['admin']['orders']['modify_status'];?></strong><br />
        <?php echo $lang['admin']['orders']['send_invoice_desc']; ?> </td>
      <td class="copyText"><select name="status">
          <?php
    ...
    <tr>
      <td width="250" class="copyText"><strong><?php echo $lang['admin']['orders']['staff_comments'];?></strong><br />
        <?php echo $lang['admin']['orders']['info_not_sent_customer'];?> </td>
    ...
    <tr>
      <td width="250" class="copyText"><strong><?php echo $lang['admin']['orders']['inform_customer'];?></strong><br />
        <?php echo $lang['admin']['orders']['customer_notified'];?> </td>
    ...
    <tr>
      ...
      <td><input type="hidden" name="email" value="<?php echo $result[0]['email']; ?>" />
