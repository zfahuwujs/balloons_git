...
if(!isset($_GET['mode'])){
	...
	if(isset($_GET['edit']) && $_GET['edit']>0){
		...
		$query = sprintf("SELECT * FROM ".$glob['dbprefix']."CubeCart_admin_users WHERE adminId = %s", $db->mySQLSafe($_GET['edit'])." AND mouAdmin != 1"); 

	} else {

	

		$query = "SELECT * FROM ".$glob['dbprefix']."CubeCart_admin_users WHERE mouAdmin != 1 ORDER BY isSuper DESC";
...
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    ...
    <td nowrap='nowrap'><p class="pageTitle">Admin Accounts</p></td>

     <?php if(!isset($_GET["mode"]) && permission("users","add")==TRUE){ ?><td align="right" valign="middle"><a href="?mode=new" class="addNew"><?php echo $lang['admin']['add_new'];?></a></td><?php } ?>
...
if($results){
	...
	<?php } 
		...
		<td colspan="6" class="tdEven"><span class="copyText">No Administrators added.</span></td>
...
<table border="0" cellspacing="0" cellpadding="3" class="mainTable" width="100%">
  ...
  <tr>
    ...
    <td colspan="2" class="tdTitle"><?php if(isset($_GET["edit"]) && $_GET["edit"]>0){ echo $modeTxt; } else { echo $modeTxt;  } ?> Add Administrator</td>
  ...
  <tr>
    ...
    <td class="tdText"><strong><?php echo $lang['admin']['adminusers']['password']; ?></strong></td>
  ...
  <tr>
    ...
    <td class="tdText"><strong><?php echo $lang['admin']['adminusers']['make_super']; ?></strong></td>
