...
$delete = $db->delete($glob['dbprefix']."CubeCart_news_lang", $where, ""); 
...
$update =$db->update($glob['dbprefix']."CubeCart_news_lang", $record, $where);
...
$insert = $db->insert($glob['dbprefix']."CubeCart_news_lang", $record);
...
if(!isset($_GET['mode'])){
	...
	if(isset($_GET['edit']) && $_GET['edit']>0){
		$query = sprintf("SELECT * FROM ".$glob['dbprefix']."CubeCart_news_lang WHERE id = %s", $db->mySQLSafe($_GET['edit'])); 
	} else {
		$query = sprintf("SELECT * FROM ".$glob['dbprefix']."CubeCart_news_lang WHERE doc_master_id = %s ORDER BY doc_name ASC", $db->mySQLSafe($_GET['doc_master_id'])); 
	} 
...
<?php 
		...
		$query = sprintf("SELECT * FROM ".$glob['dbprefix']."CubeCart_news WHERE doc_id = %s", $db->mySQLSafe($_GET['doc_master_id']));
		$results = $db->select($query);
		
		}
		?>

<form action="<?php echo $GLOBALS['rootRel'];?>admin/docs/languagesNews.php?doc_master_id=<?php echo $_GET['doc_master_id']; ?>" target="_self" method="post" language="javascript">
