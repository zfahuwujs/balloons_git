...
if(isset($_GET["delete"]) && $_GET["delete"]>0){
		...
		if($delete == TRUE){
			...
			$delRec["type"] = $db->mySQLSafe("Category");
...
/*	
	...
	// start mod: Category Descriptions
	$record["cat_desc"] = $db->mySQLSafe($_POST['FCKeditor']);		
	// end mod: Category Descriptions	
	$record["cat_father_id"] = $db->mySQLSafe($_POST['cat_father_id']);	
	$record["cat_image"] = $db->mySQLSafe(preg_replace('/thumb_/i', '', $_POST['image']));
	...
	$record["displayHomePage"] = $db->mySQLSafe($_POST['displayHomePage']);
	$record["showLeft"] = $db->mySQLSafe($_POST['showLeft']);
	...
	if($_POST['cat_id']>0) {
		...
		if($_POST['oldFatherId'] == "" || $_POST['oldFatherId'] != $_POST['cat_father_id']) {
		...
		if($update == TRUE){
			...
			$updRec["type"] = $db->mySQLSafe("Category");
			$updRec["aspect_id"] = $db->mySQLSafe($_POST['cat_id']);
			...
			$update = $db->update($glob['dbprefix']."CubeCart_category", $modRec, $where);
			/// END Part #5
		...
		if($insert == TRUE){
			...
			$insRec["type"] = $db->mySQLSafe("Category");
...
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <td align="right" valign="middle"><a <?php if(permission("categories","write")==TRUE){?>href="?mode=new"<?php } else { echo $link401; } ?> class="addNew"><?php echo $lang['admin']['add_new'];?></a></td>
    <?php } ?>
  </tr>
</table>
...
<table width="100%"  border="0" cellspacing="0" cellpadding="4" class="mainTable">
  ...
  <tr>
    ...
    <td align="center" valign="middle" nowrap='nowrap'  class="<?php echo $cellColor; ?>"><?php 
    ...
    <td align="center" width="10%" class="<?php echo $cellColor; ?> <?php evoHideAlt(102); ?>"><a <?php if(permission("categories","edit")==TRUE){ ?>href="languages.php?cat_master_id=<?php echo $results[$i]['cat_id']; ?>" class="txtLink"<?php } else { echo $link401; } ?> ><?php echo $lang['admin']['docs']['languages']; ?></a></td>
    ...
    <td align="center" width="5%" class="<?php echo $cellColor; ?>"><?php if(permission("categories","delete")==TRUE && $results[$i]['noProducts']<=0 && $results[$i]['locked']==0 ){ ?>
      <a href="javascript:decision('<?php echo $lang['admin']['delete_q']; ?>','?delete=<?php echo $results[$i]['cat_id']; ?>&amp;cat_father_id=<?php echo $results[$i]['cat_id'];?>');" class="txtLink"	><img src="/admin/images/del.gif" alt="Delete" title="Delete" border="0" /></a>
      <?php }else{echo'<img src="/admin/images/padlock.png" alt="Locked" title="Locked" width="12" border="0" />';} ?></td>
  </tr>
  <?php } // end loop
  } else { ?>
  <tr>
  ...
  <table border="0" cellspacing="0" cellpadding="3" class="mainTable" width="100%">
    <tr>
      <td colspan="2" class="tdTitle"><?php if(isset($_GET["edit"]) && $_GET["edit"]>0){ echo $modeTxt; } else { echo $modeTxt;  } ?>
        <?php echo $lang['admin']['categories']['category'];?></td>
    </tr>
    <tr>
      <td class="tdText" width="15%"><?php echo $lang['admin']['categories']['category_name'];?></td>
      <td><input name="cat_name" type="text" class="textbox" value="<?php if(isset($results[0]['cat_name'])) echo validHTML($results[0]['cat_name']); ?>" maxlength="255" /></td>
    </tr>
    <!-- start mod: Category Descriptions -->
    <tr>
      <td class="tdText"> Category Description: </td>
      <td class="tdText" width="800"><?php
...
?></td>
    ...
    <!-- end mod: Category Descriptions -->
    <tr>
      <td class="tdText">Include In Navigation:</td>
      <td><select name="menuShow" class="textbox">
     ...
     <tr>
      <td class="tdText">Include In Left Column:</td>
      <td><label for="showLeft"></label>
        <select name="showLeft" id="showLeft" class="textbox">
          <option value="1" <?php if($results[0]['showLeft'] == 1) echo "selected"; ?>>Yes</option>
          <option value="0" <?php if($results[0]['showLeft'] == 0) echo "selected"; ?>>No</option>
      </select></td>
    </tr>
    <tr>
      <td class="tdText">Show on home page under slider:</td>
      <td><label for="displayHomePage"></label>
        <select name="displayHomePage" id="displayHomePage" class="textbox">
          <option value="1" <?php if($results[0]['displayHomePage'] == 1) echo "selected"; ?>>Yes</option>
          <option value="0" <?php if($results[0]['displayHomePage'] == 0) echo "selected"; ?>>No</option>
      </select></td>
    </tr>
    <tr>
	...
	?>
  	...
  	for ($i=0; $i<count($categories); $i++){
		...
		if($categories[$i]['cat_id']!==$results[0]['cat_id']){ ?>
          <option value="<?php echo $categories[$i]['cat_id']; ?>" <?php if(isset($results[0]['cat_father_id']) && $categories[$i]['cat_id']==$results[0]['cat_father_id']) { echo "selected='selected'"; } ?>> <?php echo getCatDir($categories[$i]['cat_name'],$categories[$i]['cat_father_id'], $categories[$i]['cat_id']); ?> </option>
          <?php
    ...
    <tr <?php evoHide(44); ?>>
      ...
      <td valign="top"><div id="selectedImage">
          <?php if(!empty($results[0]['cat_image'])){ ?>
          <img src="<?php echo $GLOBALS['rootRel']."images/uploads/".$results[0]['cat_image']; ?>" alt="<?php echo $results[0]['cat_image']; ?>" title="" />
        ...
        <div id="imageControls">
          <table border="0" cellspacing="0" cellpadding="3">
            ...
            <tr>
              <td><input name="browse" class="submit" type="button" id="browse" onclick="openPopUp('../filemanager/browse.php?custom=1&amp;cat=5','filemanager',450,500)" value="Browse Images" /></td>
            </tr>
          </table>
        </div>
        <input type="hidden" name="image" id="imageName" value="<?php echo $results[0]['cat_image']; ?>" />
      </td>
  ...
  ?>
    ...
    <tr>
      <td colspan="2"><p>
        ...
        <table width="100%"  border="0" cellspacing="0" cellpadding="4" class="mainTable">
          <tr>
            <td colspan="2" class="tdTitle"><strong>SEO Settings</strong></td>
          </tr>
          <?php if($config['sefcustomurl'] == 1) { ?>
          <tr>
            <td width="30%" align="left" valign="top" class="tdText"><strong>Custom URL:</strong></td>
            <td align="left"><?php echo $glob['storeURL']; ?>/
              <input name="cat_sefurl" type="text" size="20" class="textbox" value="<?php if(isset($results[0]['cat_sefurl'])) echo $results[0]['cat_sefurl']; ?>" />
              /c_xx.html</td>
          </tr>
          <?php } ?>
          <tr>
            <td width="30%" class="tdText"><strong>Page Title:</strong></td>
          ...
          <tr>
            ...
            <td align="left"><textarea name="cat_metadesc" cols="35" rows="3" class="textbox"><?php if(isset($results[0]['cat_metadesc'])) echo $results[0]['cat_metadesc']; ?>
</textarea></td>
          </tr>
          <tr>
            ...
            <td align="left"><textarea name="cat_metakeywords" cols="35" rows="3" class="textbox"><?php if(isset($results[0]['cat_metakeywords'])) echo $results[0]['cat_metakeywords']; ?>
</textarea></td>
          </tr>
        </table>
        </p></td>
    ...
    <tr>
      <td>&nbsp;</td>
      <td><input type="hidden" name="noProducts" value="<?php echo $results[0]['noProducts']; ?>" />
