...
if(isset($_POST['add']) && $_POST['add']=="option"){
	...
	$allTop = $db->insert("SELECT * FROM ".$glob['dbprefix']."CubeCart_options_top");
	if($alltop){
		for($i=0; $i<count($allTop); $i++){
			if($i==0){
				$record['parent'] = $db->mySQLSafe(0);
			}else{
				$record['parent'] = $db->mySQLSafe($allTop[$i-1]["option_id"]);
			}
			$update = $db->update($glob['dbprefix']."CubeCart_options_top", $record, "option_id=".$db->mySQLSafe($allTop[$i]["option_id"]));
			unset($record);
		}
	}

	$record['option_name'] = $db->mySQLSafe($_POST["option"]);
	$record['parent'] = $db->mySQLSafe($allTop[count($allTop)-1]["option_id"]);
	...
	$record['ordering'] = $db->mySQLSafe($_POST["value_order"]);
...
} elseif(isset($_POST['add']) && $_POST['add']=="assign" && !isset($_POST['SubmitTwo'])){
...
} elseif(isset($_POST['OrderChange']) && isset($_POST['SubmitTwo'])){
	$query = "SELECT value_id FROM ".$glob['dbprefix']."CubeCart_options_mid ORDER BY value_id DESC LIMIT 1";

	$products = $db->select($query);

	for ($i = 1; $i <= $products[0]['value_id']; $i++){
		$Field = "order$i";
	
		if (isset($_POST[$Field])) {
			$where = "value_id = ".$db->mySQLSafe($i);
			$record['ordering'] = $db->mySQLSafe($_POST[$Field]);
			$update = $db->update($glob['dbprefix']."CubeCart_options_mid", $record, $where);
		}
	}
...
/////////
	...
	if(isset($_GET['editAssigned']) && $_GET['editAssigned']>0){
		...
		//Check if items are to be filtered by attribute
		
		if(isset($_POST['filter'])){
			$query = "SELECT * FROM ".$glob['dbprefix']."CubeCart_options_bot INNER JOIN ".$glob['dbprefix']."CubeCart_inventory ON product = productId WHERE productId = ".$db->mySQLSafe($_POST['productId'])." ORDER BY name, option_id, value_id ASC";
		}else{
...
<script type="text/javascript">
	$(function() {
		...
		$.ajax({
			type: 'get',
			url: 'getOptions.php',
			data: 'option=' + $('.attribute').attr('value'),
			success: function(msg) {
				$('#optionsList').html(msg);
...
<form name="quickAssign" method="post" action="<?php echo urldecode($currentPage); ?>">
  ...
  <tr>
    ...
    <td width="50%" valign="top" class="tdText"><table width="100%"  border="0" cellspacing="0" cellpadding="0">
	 ...
	 <table width="100%"  border="0" cellspacing="0" cellpadding="4" class="mainTable">
      ...
      <tr align="center">
        ...
        <td class="tdTitle">Parent</td>

        <td class="tdTitle">Edit</td>
	  ...
	  <tr align="center">
        ...
        <td class="<?php echo $cellColor; ?>"><?php echo $options[$i]['parent']; ?></td>

        <td class="<?php echo $cellColor; ?>"><a <?php if(permission("products","edit")==TRUE){ ?>href="?editOption=<?php echo $options[$i]['option_id'];?>&amp;redir=<?php echo $currentPage; ?>"<?php } else { echo $link401; } ?> class="txtLink"><img src="/admin/images/edit.gif" alt="Edit" title="Edit" width="16" border="0" /></a></td>
        <td class="<?php echo $cellColor; ?>"><a <?php if(permission("products","delete")==TRUE){ ?>href="javascript:decision('<?php echo $lang['admin']['products']['warn_remove_opt'];?>','?delOption=<?php echo $options[$i]['option_id']; ?>&amp;redir=<?php echo $currentPage; ?>');" class="txtLink"<?php } else { echo $link401; } ?>><img src="/admin/images/del.gif" alt="Delete" title="Delete" border="0" /></a> </td>
	  ...
	  <tr align="center">
        ...
        <td>
        	<select name="parent" class="textbox">
            	<?php
					$parents=$db->select("SELECT * FROM ".$glob['dbprefix']."CubeCart_options_top");
					for($op=0; $op<count($parents); $op++){
						if(!isset($_GET['editOption']) ||(isset($_GET['editOption']) && $_GET['editOption']!=$parents[$op]['option_id'])){
							echo '<option value="'.$parents[$op]['option_id'].'">'.$parents[$op]['option_name'].'</option>';
						}
					}
				?>
            </select>
	...
	<table width="100%"  border="0" cellspacing="0" cellpadding="4" class="mainTable">
	  ...
	  <tr align="center">
        ...
        <input name="order<?php echo $attributes[$i]['value_id'];?>" type="text" class="textbox" id="order" size="1" value="<?php echo $attributes[$i]['ordering'];?>" />
        </td>
        ...
        <td class="<?php echo $cellColor; ?>"><a <?php if(permission("products","edit")==TRUE){ ?>href="?editAttribute=<?php echo $attributes[$i]['value_id']; ?>&amp;redir=<?php echo $currentPage; ?>"<?php } else { echo $link401; } ?> class="txtLink"><img src="/admin/images/edit.gif" alt="Edit" title="Edit" width="16" border="0" /></a></td>
	  ...
	  <tr>
        <td colspan="6">
        <input type="submit" name="SubmitTwo" value="Update List Order" class="submit" />
        <input type="hidden" name="OrderChange" value="<?php echo $i;?>" /><?php if ($OrdersUpdate){ echo '<span style="color: #00cc00">List Orders have been updated</span>';}?>
        </form>
        </td>
    </tr>
      ...
      <tr align="center">
		...
		<td><input type="text" name="value_order" size="4" maxlength="4" value="<?php if(isset($attributes[0]['ordering']) && isset($_GET['editAttribute'])){ echo $attributes[0]['ordering']; }else{ echo "0";}?>" /></td>
  ...
  <tr valign="top">
	...
	<?php if($attributes == TRUE) { ?>
    <form name="filter" method="post" action="<?php echo urldecode($currentPage); ?>">
    
	<table border="0" cellspacing="2" cellpadding="0">

        <tr>

          <td height="25"><strong>Fitler Products:</strong></td>
	...
	<table width="100%"  border="0" cellspacing="0" cellpadding="4" class="mainTable">
	  ...
	  <tr align="center">
        ...
        <td class="<?php echo $cellColor; ?>"><a <?php if(permission("products","edit")==TRUE){ ?>href="?editAssigned=<?php echo $existingOptions[$i]['assign_id']; ?>&amp;redir=<?php echo $currentPage; ?>"<?php } else { echo $link401; } ?> class="txtLink"><img src="/admin/images/edit.gif" alt="Edit" title="Edit" width="16" border="0" /></a></td>
...
<?php } // end loop ?>
      ...
      <tr align="center">
		...
		<select name="option" id="option" class="textbox attribute">
