...
function getCatDir($catName, $cat_father_id, $catId, $link = FALSE, $skipFirstSymbol = FALSE, $reverseSort = TRUE) {
    ...
    $query = "SELECT cat_id, cat_name, cat_father_id FROM " . $glob['dbprefix'] . "CubeCart_category WHERE cat_id = " . $cat_father_id . " ORDER BY cat_id DESC";
    ...
    for ($i = 0; $i <= count($catArray); $i++) {
        ...
        if (isset($catArray[$i]['cat_id']) && $catArray[$i]['cat_id'] == $cat_father_id) {
            ...
            if ($cat_father_id) {
                $query = "SELECT cat_id, cat_name, cat_father_id FROM " . $glob['dbprefix'] . "CubeCart_category WHERE cat_id = " . $cat_father_id . " ORDER BY cat_id DESC";
                $fatherCat = $db->select($query);
                $catArray = array_merge($catArray, $fatherCat);
            }
        }
    }
...
function readableSeconds($time = 0) {
    ...
    if ($output and $minutes > 0 and $seconds > 0)
        $output .= ", ";

    else if ($output and $minutes > 0 and $seconds == 0)
        $output .= " and ";
