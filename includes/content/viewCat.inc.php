...
if ($productResults == TRUE) {
	...
	for ($i = 0; $i < count($productResults); $i++) {	
		...
		if($ccUserData[0]['customer_id'] > 0 && $ccUserData[0]['trade'] > 0){
					...
					$productResults[$i]['tradePrice'] = $productResults[$i]['price'] - ($productResults[$i]['price'] * $discount);
					$productResults[$i]['tradeSale'] = $productResults[$i]['sale_price'] - ($productResults[$i]['sale_price'] * $discount);
					...
					if($ccUserData[0]['tax'] == 2){
						if(salePrice($productResults[$i]['tradePrice'], $productResults[$i]['tradeSale'])==FALSE){
							$vatTradePrice = getProductPriceWithTax($productResults[$i]['productId'], 'price');
							...
							$vatTradePrice = getProductPriceWithTax($productResults[$i]['productId'], 'price');
							$vatTradePrice = $vatTradePrice - ($vatTradePrice * $discount);
							$view_cat->assign("TXT_PRICE","<strike>".priceFormat($vatTradePrice)."</strike>");
