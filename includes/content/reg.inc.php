...
if(isset($_POST['email'])){
	...
	$emailArray = $db->select("SELECT customer_id, type FROM ".$glob['dbprefix']."CubeCart_customer WHERE ".$usernameType."=".$db->mySQLSafe($_POST[$usernameType]));
	...
	if(empty($_POST['firstName']) || empty($_POST['lastName']) || empty($_POST['email']) || empty($_POST['phone']) || empty($_POST['add_1']) || empty($_POST['town']) || empty($_POST['county']) || empty($_POST['postcode']) || empty($_POST['country']) || empty($_POST['password']) || empty($_POST['passwordConf'])){
	...
	} elseif(validateEmail($_POST['emailConf'])==FALSE) {

		$errorMsg = $lang['front']['reg']['enter_valid_email'];
		
	} elseif($_POST['email']!=$_POST['emailConf']) {

		$errorMsg = "Your email address and confirmed email address do not match";
	...
	} elseif($config['usernameType']==1 && $_POST['nickname'] != '') {
		...
		if($config['usernameType']==1){
			$record["nickname"] = $db->mySQLSafe($_POST['nickname']);
		}
...
if($config['usernameType']==1){
	...
	$reg->assign("LANG_COMPANY",$lang['front']['reg']['company']);
	...
	$reg->assign("STORE_NAME", $config['storeName']);
	...
	if(isset($_POST['title'])){
		...
		if($config['usernameType']==1){
			$reg->assign("VAL_NICKNAME",treatGet($_POST['nickname']));
		}
		...
		$reg->assign("VAL_EMAIL_CONF",treatGet($_POST['emailConf']));
		...
		$reg->assign("VAL_COMPANY",treatGet($_POST['company']));
		...
		if(isset($_POST['tradeAccount']) && $_POST['tradeAccount']==1) {
			...
			$reg->assign("VAL_TRADE_CHECKED","checked='checked'");
